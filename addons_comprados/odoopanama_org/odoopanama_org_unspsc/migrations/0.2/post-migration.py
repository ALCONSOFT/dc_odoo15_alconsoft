...
def migrate(cr, version):
    """This code was added in the `panama.unspsc.code.csv` file
    but that file is loaded only when the `odoopanama_org_unspsc` module is installed (by hook).
    ...
    ctx = {'current_module': 'odoopanama_org_unspsc', 'noupdate': True}
    res = env['panama.unspsc.code'].with_context(ctx).load(
