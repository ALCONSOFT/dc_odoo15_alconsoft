...
class AccountMove(models.Model):
    ...
    def get_data_from_origin_move_id(origin_move_id):
        ...
        if origin_move_id and origin_move_id.l10n_latam_document_type_id:
            number = origin_move_id.name and origin_move_id.name.replace(
                ' ', '')
...
class AccountDebitNote(models.TransientModel):
    ...
    def _prepare_default_values(self, move):
        default_values = super(
            AccountDebitNote, self)._prepare_default_values(move)
        if default_values['debit_origin_id']:
            debit_origin_id = self.env['account.move'].browse(
                default_values['debit_origin_id'])
            document_type, invoice_date, number = move.get_data_from_origin_move_id(
                debit_origin_id)
